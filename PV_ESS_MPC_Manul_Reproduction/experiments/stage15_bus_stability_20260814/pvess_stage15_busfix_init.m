...
% Constant-power-load validation settings. The DC implementation keeps the
% existing SPS Variable Resistor hardware and updates R=Vdc^2/P digitally.
% A one-sample delay breaks the electrical algebraic loop and represents a
% realistic sampled load controller; the voltage floor is a numerical and
% over-current protection boundary, not a clamp on the DC-bus waveform.
Ts_cpl = 1/20e3;              % [s] 50 us, aligned with the MPC update
Vdc_cpl_floor = 0.70*Vdc_ref; % [V] denominator/protection floor
Rdc_cpl_min = 0.2;            % [ohm] matches SPS Variable Resistor limit
Rdc_cpl_off = 1e6;            % [ohm] practical open circuit
Pdc_cpl_eps = 100;            % [W] off-state decision and divide floor
...
% The former values implemented an approximately 20 Hz, zeta=0.707 loop.
% Under the genuine 1 MW DC constant-power step this required roughly
% 260 V of proportional error before commanding a 1 MW power reversal.
% A 20/30/40/50 Hz sweep with unchanged Cdc, load, and converter ratings
% selected 50 Hz: 10.72 ms recovery, no grid-current limiting, and the
% smallest voltage dip without increasing steady post-event ripple.
f_bw_dc = 50;                       % [Hz] validated outer-loop bandwidth
zeta_dc = 1/sqrt(2);                % [-] retained Butterworth damping
Kp_dc = 2*zeta_dc*(2*pi*f_bw_dc)*Cdc*Vdc_ref; % [W/V]
Ki_dc = (2*pi*f_bw_dc)^2*Cdc*Vdc_ref;          % [W/(V*s)]
Pref_min = -2.2e6;
Pref_max = 2.2e6;
P_base_mpc = 2e6;
Q_base_mpc = 1e6;
...
% Commanded powers for the executable constant-power DC-load branch.
Pdc1_cpl_profile = timeseries([0;0;P_dc_1], ...
    [0;T_pv_ramp_up;T_pv_ramp_up+Ts_power]);
Pdc2_cpl_profile = timeseries([0;0;P_dc_2;P_dc_2;0], ...
    [0;T_dc_2_on;T_dc_2_on+Ts_power;T_pv_ramp_down;T_pv_ramp_down+Ts_power]);
...
stage15_fix_id = 'stage15_engineering_limits_tracking_grid_pq_acdc_cpl_20260817';
